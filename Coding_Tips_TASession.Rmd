...
3. **General Tips for Research and Writing**
...
p <- ggplot(dffull,aes(y=Y,x=X,color=as.factor(Z)))+geom_point(size=3)+
  theme_minimal() +
  geom_vline(aes(xintercept=mean_X,color=as.factor(Z)), size = 2)+
  geom_hline(aes(yintercept=mean_Y,color=as.factor(Z)), size = 2)+
  ...
  geom_segment(aes(x=ifelse(time==afterlab,endpts$mean_X[1],NA),
                   ...
                   yend=endpts$mean_Y[2]),size=2,color='blue')+
...
msummary(list('Income (First Stage)' = m_iv$iv_first_stage[[1]],
              'Expenditure' = m_iv), coef_map = c('fit_income' = 'Income (2SLS)',
...
$$
E[Y | C, A] - E[Y | C, B] = Time
$$
- Before-After Difference for Trmt Group: 
$$
E[Y | T, A] - E[Y | T, B] = Time + Trmt
$$
- Difference-in-Differences:
$$
(E[Y | T, A] - E[Y | T, B]) - (E[Y | C, A] - E[Y | C, B]) = Trmt
$$
...
p <- ggplot(dffull,aes(y=Y,x=xaxisTime,color=as.factor(Control)))+geom_point(size=3)+
  ...
  geom_segment(aes(x=ifelse(state %in% c('2a. Take means by group and time.',"2b. Take means by group and time."),
                   ...
                   yend=filter(dfseg,Time=='Before',Control=='Control')$mean_Y[1]),size=2,color='black')+
  geom_segment(aes(x=ifelse(state %in% c('2a. Take means by group and time.',"2b. Take means by group and time."),
                            .5,NA),
                   xend=1.5,y=filter(dfseg,Time=='Before',Control=='Treatment')$mean_Y[1],
                   yend=filter(dfseg,Time=='Before',Control=='Treatment')$mean_Y[1]),size=2,color="#E69F00")+
  geom_segment(aes(x=ifelse(state %in% c('2a. Take means by group and time.',"2b. Take means by group and time."),
                            1.5,NA),
                   xend=2.5,y=filter(dfseg,Time=='After',Control=='Control')$mean_Y[1],
                   yend=filter(dfseg,Time=='After',Control=='Control')$mean_Y[1]),size=2,color='black')+
  geom_segment(aes(x=ifelse(state %in% c('2a. Take means by group and time.',"2b. Take means by group and time."),
                            1.5,NA),
                   xend=2.5,y=filter(dfseg,Time=='After',Control=='Treatment')$mean_Y[1],
                   yend=filter(dfseg,Time=='After',Control=='Treatment')$mean_Y[1]),size=2,color="#E69F00")+
  ...
  geom_segment(aes(x=1.5,xend=1.5,
                   ...
                   yend=filter(dfseg,Time=='Before',Control=='Treatment')$mean_Y[1]),size=2,color='blue')+
  ...
  geom_segment(aes(x=1.5,xend=1.5,
                   ...
                   yend=filter(dfseg,Time=='Before',Control=='Control')$mean_Y[1]),size=2,color='blue')+
  labs(title = '')+
...
$$ Y = \gamma_i TrmtGroup + \gamma_t After + \beta TrmtGroup\times After + \varepsilon $$
